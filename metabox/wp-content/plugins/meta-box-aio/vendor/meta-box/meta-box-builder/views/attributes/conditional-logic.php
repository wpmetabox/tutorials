<label>
	<a href="https://docs.metabox.io/extensions/meta-box-conditional-logic/" target="_blank"><?php esc_html_e( 'Conditional Logic', 'meta-box-builder' ) ?></a>
	<?= mbb_tooltip( __( 'Toggle the field based on value of other fields, terms or HTML elements', 'meta-box-builder' ) ) ?>
</label>
<div ng-show="active.logic.when.length > 0">
	<header class="mbb-settings-header">
		<select ng-model="active.logic.visibility">
		...
		<select ng-model="active.logic.relation">
	...
	<table class="mbb-settings-table">
		<tr ng-repeat="item in active.logic.when track by $index">
			<td>
				<input type="text" ng-model="active.logic.when[$index][0]" list="available_fields" placeholder="<?php esc_attr_e( 'Select or enter a field', 'meta-box-builder' ) ?>">
			</td>
			<td>
				<select style="width: 80px;" ng-model="active.logic.when[$index][1]">
					<option value="=">=</option>
					<option value=">">&gt;</option>
					<option value="<">&lt;</option>
					<option value=">=">&gt;=</option>
			...
			<td>
				<input type="text" ng-model="active.logic.when[$index][2]" placeholder="<?php esc_attr_e( 'Enter a value', 'meta-box-builder' ) ?>">
			</td>
			<td>
				<button type="button" class="button mbb-button-delete" ng-click="removeConditionalLogic($index);"><span class="dashicons dashicons-dismiss"></span></button>
			</td>
		</tr>
	</table>
</div>

<button type="button" class="button" ng-click="addConditionalLogic();"><?php esc_html_e( '+ Add Rule', 'meta-box-builder' ) ?></button>
